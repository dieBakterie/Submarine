...
execute as @s[nbt={SelectedItemSlot:0}] if block ^ ^0.5 ^1.5 #submarine:water run tag @e[name=sub,distance=..2,limit=1,scores={subenergy=1..}] add subforward
execute as @s[nbt={SelectedItemSlot:2}] if block ^ ^0.5 ^-1.5 #submarine:water run tag @e[name=sub,distance=..2,limit=1,scores={subenergy=1..}] add subbackwards
