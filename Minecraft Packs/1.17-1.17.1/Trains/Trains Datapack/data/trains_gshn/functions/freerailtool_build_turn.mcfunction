execute if block ~ ~-3 ~ minecraft:chain_command_block{CustomName:'{"text":"rptrain_freerailtool_trigger"}'} run tag @s add rptrain_turn-3
...
execute unless entity @s[tag=rptrain_turn-3] run setblock ~ ~-3 ~ minecraft:chain_command_block[conditional=true,facing=down]{Command:"Do not press ENTER or the DONE button below! - Gshn28",SuccessCount:-1,LastExecution:1,CustomName:'{"text":"rptrain_freerailtool_trigger"}'} replace
...
execute if entity @s[tag=rptrain_turn-3] run setblock ~ ~-4 ~ minecraft:chain_command_block[conditional=true,facing=down]{Command:"Do not press ENTER or the DONE button below! - Gshn28",SuccessCount:-1,LastExecution:1,CustomName:'{"text":"rptrain_freerailtool_trigger"}'} replace
